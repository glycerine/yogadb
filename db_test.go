...
var errMemWALWriteForTest = errors.New("injected memwal write failure")

type failWriteAtFileForTest struct {
	vfs.File
	failWriteAt bool
}

func (f *failWriteAtFileForTest) WriteAt(p []byte, off int64) (int, error) {
	if f.failWriteAt {
		return 0, errMemWALWriteForTest
	}
...
func TestFlexDB_kv128RoundTrip(t *testing.T) {
	...
	for _, kv := range cases {
		...
		if len(buf) != kv128EncodedSize(kv) {
			t.Fatalf("size mismatch: encoded %d, predicted %d for kv='%#v'", len(buf), kv128EncodedSize(kv), kv)
		}
		...
		if got.HasVPtr() != kv.HasVPtr() {
			t.Fatalf("HasVPtr mismatch: got %v, want %v; \n\n kv='%#v'", got.HasVPtr(), kv.HasVPtr(), kv)
		}
...
func TestFlexDB_GetKVReportsAccurateVptrLength(t *testing.T) {
	...
	if err := batch.Set("small-batch", smallBatch, 0); err != nil {
		t.Fatal(err)
	}
...
func TestFlexDB_MergePersistence(t *testing.T) {
	...
	err := db.Merge("k1", func(old []byte, exists bool, oldVtyp uint64) (newValue []byte, doWrite bool, doDelete bool, newVtyp uint64) {
...
func TestLen(t *testing.T) {
	...
	t.Run("persistence", func(t *testing.T) {
		...
		for i := 0; i < N; i++ {
			if i%3 == 0 {
				db.Put(fmt.Sprintf("key%04d", i), largeVal, 0)
			} else {
				db.Put(fmt.Sprintf("key%04d", i), []byte("small"), 0)
			}
		}
		// Delete a few
		db.Delete("key0003")
...
func TestFlexDB_ZeroLengthValueEquivalence(t *testing.T) {
	...
	if vtyp1 != vtyp0 {
		t.Fatalf("expected vtyp1(%v) to equal vtyp0(%v)", vtyp1, vtyp0)
	}
}
