...
// IsNil uses reflect to return true iff the face
...
func debug_assert(wantTrue bool) {
	if !wantTrue {
		panic("debugAssert failed")
	}
}

func static_assert(wantTrue bool, msg string) {
	if !wantTrue {
		panicf("static_assert failed: '%v'", msg)
	}
}
