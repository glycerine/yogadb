...
// Tests upsidedown/yogadb versus upsidedown/boltdb against various templated
// queries. This is vendored from Bleve's scorch-vs-upsidedown suite and adapted
// to isolate the KVStore backend.
func TestUpsideDownBoltVersusYogaDBAll(t *testing.T) {
	(&VersusTest{
		t:                    t,
		NumDocs:              1000,
		MaxWordsPerDoc:       20,
	...
	}).run(upsidedown.Name, boltdb.Name, upsidedown.Name, yogableve.Name, nil, nil)
}

func TestUpsideDownBoltVersusYogaDBSmallMNSAM(t *testing.T) {
	(&VersusTest{
		t:                    t,
...
func TestUpsideDownBoltVersusYogaDBSmallCMP11(t *testing.T) {
	(&VersusTest{
		t:                    t,
...
func (vt *VersusTest) run(indexTypeA, kvStoreA, indexTypeB, kvStoreB string,
	...
	kvConfigA := kvConfig(kvStoreA)
	kvConfigB := kvConfig(kvStoreB)
...
func kvConfig(store string) map[string]interface{} {
	cfg := map[string]interface{}{}
	if store == yogableve.Name {
		cfg["disableBackgroundFlush"] = true
	}
	return cfg
}
