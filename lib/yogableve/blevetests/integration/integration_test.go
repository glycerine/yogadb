...
var indexType = flag.String("indexType", upsidedown.Name, "index type to build")
var kvType = flag.String("kvType", yogableve.Name, "kv store type to build")
...
func TestIntegration(t *testing.T) {
	...
	for _, f := range entries {
		...
		if f.IsDir() {
			if reason := skipDataset(f.Name()); reason != "" {
				t.Logf("Skipping test: %s (%s)", f.Name(), reason)
				continue
			}
...
func skipDataset(name string) string {
	if name == "geoshapes" && *indexType == upsidedown.Name {
		return "Bleve's upsidedown indexer does not implement geoshape search; run with -indexType scorch to exercise this fixture"
	}
	return ""
}
...
func loadDataSet(t *testing.T, datasetName string, mapping mapping.IndexMappingImpl, path string) (bleve.Index, func(), error) {
	idxPath, err := integrationIndexPath(t, datasetName)
	if err != nil {
		return nil, nil, err
	}
...
func integrationIndexPath(t *testing.T, datasetName string) (string, error) {
	name := fmt.Sprintf("test-%s.bleve", datasetName)
	if *keepIndex {
		dir, err := os.MkdirTemp("", "yogableve-bleve-integration-")
		if err != nil {
			return "", fmt.Errorf("error creating keepIndex temp dir: %v", err)
		}
		t.Logf("keeping index under %s", dir)
