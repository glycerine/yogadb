...
func TestAllowReadsSyncsPreReadBulkLoad(t *testing.T) {
	...
	if db.mt.activeLen() != 0 {
		t.Fatalf("active memtable length after AllowReads = %d, want 0", db.mt.activeLen())
	}
	if !db.mt.empty.Load() {
...
func TestReopenedExistingDBBatchSetDeleteBeforeAllowReadsMergesAtAllowReads(t *testing.T) {
	...
	if db.mt.activeLen() != 0 {
		t.Fatalf("reopened pre-AllowReads batch should not use normal memtable path: active memtable length=%d, want 0", db.mt.activeLen())
	}
