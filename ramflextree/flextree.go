...
var freedLeaf = LeafNode{CommonNode: CommonNode{Freed: true}} //, Next: IllegalID, Prev: IllegalID}
var freedInternal = InternalNode{CommonNode: CommonNode{Freed: true}}
...
type InternalNode struct {
	...
	// given n pivots,
	Pivots [FLEXTREE_INTERNAL_CAP]uint64
	// we need n+1 children.
	Children [FLEXTREE_INTERNAL_CAP_PLUS_ONE]InternalChild
}
...
type LeafNode struct {
	...
	Extents [FLEXTREE_LEAF_CAP]FlexTreeExtent
...
type FlexTree struct {
	...
	LeafHead NodeID // linked list head

	// the arenas that hold all nodes.
	InternalArena []InternalNode
	LeafArena     []LeafNode
...
type FlexTreePath struct {
	Level uint8
	Path  [FLEXTREE_PATH_DEPTH]uint8
	Nodes [FLEXTREE_PATH_DEPTH]NodeID
}
...
type Pos struct {
	...
	Loff uint64 // logical offset of the position
	Idx  uint32 // index of the entry
	Diff uint32 // the current position within the entry
}
...
func (t *FlexTree) AllocInternal() (ie *InternalNode) {
	...
	*ie = initInternalValue // starts IllegalID on all Children
...
func (ie *InternalNode) StringOnTree(tree *FlexTree) (r string) {
	...
	if ie.Count > 0 {
		...
		for i := uint32(0); i < ie.Count+1; i++ {
			...
			fmt.Fprintf(buf, "  [child %02d] %v NodeID %v shift %d\n",
				i, typ, ie.Children[i].NodeID, ie.Children[i].Shift)
		}
		for i := uint32(0); i < ie.Count+1; i++ {
