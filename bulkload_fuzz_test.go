...
func FuzzBulkLoadBeforeAllowReads(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		commitPending := func(doFsync bool) {
			...
			if got := db.mt.activeLen(); got != 0 {
				t.Fatalf("pre-AllowReads fast batch used normal memtable: active memtable length=%d", got)
			}
		...
		for i, ops := 0, 0; i < len(data) && ops < 128; ops++ {
			...
			case 4: // Sync before AllowReads; this must not enable reads.
				...
				if got := db.mt.activeLen(); got != 0 {
					t.Fatalf("pre-AllowReads Sync left normal memtable entries: active memtable length=%d", got)
				}
			}
		}
...
func FuzzBulkLoadReloadBeforeAllowReads(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		commitPending := func(doFsync bool) {
			...
			if got := db.mt.activeLen(); got != 0 {
				t.Fatalf("second pre-AllowReads batch used normal memtable: active memtable length=%d", got)
			}
			if db.mt.bulk.count > maxBulkCount {
				maxBulkCount = db.mt.bulk.count
			}
		...
		for i, ops := 0, 0; i < len(data) && ops < 128; ops++ {
			...
			case 4:
				...
				if got := db.mt.activeLen(); got != 0 {
					t.Fatalf("second pre-AllowReads Sync left normal memtable entries: active memtable length=%d", got)
				}
			}
		}
