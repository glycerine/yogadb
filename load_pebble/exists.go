...
// WalkDir uses os.Lstat under the hood.
func DirSize(path string) (totalSize int64, err0 error) {

	err0 = filepath.WalkDir(path, func(filePath string, d fs.DirEntry, err error) error {
...
func MustDirSize(path string) int64 {
	totalSize, err := DirSize(path)
	panicOn(err)
	return totalSize
}
