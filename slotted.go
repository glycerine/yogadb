...
// Amplification with the original FlexSpace design.
//
...
func slottedPageEncodeKnownSize(dst []byte, kvs []KV, baseHLC HLC, totalSize int) []byte {
	count := len(kvs)
	if count == 0 {
		return nil
	}
	...
	if totalSize < slottedPageHeaderSize+slottedPageCRCSize {
		panic(fmt.Sprintf("slottedPageEncodeKnownSize: invalid total size %d", totalSize))
	}
	var buf []byte
	if cap(dst) >= totalSize {
		buf = dst[:totalSize]
	...
	if entryOff > valEnd {
		panic(fmt.Sprintf("slottedPageEncodeKnownSize: entry/value overlap entryOff=%d valEnd=%d total=%d", entryOff, valEnd, totalSize))
	}
...
func slottedValInfo(kv KV) uint16 {
	...
	if kv.HasVPtr() {
		if kv.Vtyp() != 0 {
			return slottedValInfoVPtrWithVtyp
		}
	...
	if kv.Vptr.Offset != 0 {
		vi |= slottedValInfoInlineVtypFlag
	}
	return vi
}
...
func slottedPageDumpImpl(src []byte, vlog *valueLog) string {
	...
	for i, e := range entries {
		...
		var valDesc string
		baseValInfo := slottedValInfoBase(e.valInfo)
		switch {
		...
		if (baseValInfo == slottedValInfoVPtr || baseValInfo == slottedValInfoVPtrWithVtyp) && scanOK {
